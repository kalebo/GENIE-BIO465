...
xout <- apply(gdsc.filtered.cn.t[2:nrow(gdsc.filtered.cn.t), ], c(1,2), extrmtrans)
table(xout)
...
```{r}
genie.cn.table <- table(unlist(genie.filtered.cn[,2:ncol(genie.filtered.cn)]))
```
```{r}
library(scales)
ggplot(as.data.frame(genie.cn.table)[-2,]) + geom_col(aes(x = Var1, y = Freq)) + scale_y_log10( breaks = trans_breaks("log10", function(x) 10^x), labels = trans_format("log10", math_format(10^.x))) + labs(x="Discrete CN Level", y ="Count", title="GENIE Copy Number Distribution")
#gdsc.cn.table <- table(unlist(genie.filtered.cn[,2:ncol(genie.filtered.cn)]))
```
