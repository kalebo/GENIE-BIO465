...
author: "Kaleb Olson, Richard Olpin, Jared Nielsen"
date: "April 13, 2017"
...
suppressWarnings(suppressPackageStartupMessages(library(synapseClient)))
...
screened.compounds <- readRDS("screened-compounds.rds")
...
colnames(genie.cancer.types) <- c("CANCER_TYPE","Count")
...
```{r}
# Here is how wer generate the filters but it takes 30-60 seconds to execute
#allFilters <- makeFilters(cancer_types,cancerPairedWithSubtypes,sexes,races,ages)

allFilters <- readRDS("allFilters.rds")
```
...
PLot the Distribution of both GENIE and GDSC CN Levels
```
library(scales)
...
Generate table of translated GDSC CN values
```{r}
...
We generalize the above as functions so that we can create profiles for an arbitrary record set and copy levels as well as finding the distance between two subsets
```{r}
...
# here is a filterd Screended.compounds of the drugs we test
drugsTested <- filter(screened.compounds,screened.compounds$`Drug ID` %in% testableDrugs$DRUG_ID)
```
...
Command we ran to generate our genie profile table. This will take 15-25 minutes to execute so just read the RDS file
```{r}
#genie.subset.cn.profiles <- suppressWarnings(makeGenieProfileTable(cnlevels, allFilters))

genie.subset.cn.profiles <- readRDS("genie.subsets.cn.profiles.rds")
```


Returns all the records for a drug that the IC_50 < threshold and has >= 100 records
```{r}
...
Generate our scoring Matrix that scores each pairing by CN similarity 
```{r}
...
drugName <- function(id){
  select(filter(screened.compounds,`Drug ID` == id), `Drug Name`)
}
...
Generate top Results Table
```
topResultsTable <- Best60Results
CSPName <- function(id){
  select(filter(allFilters,ID == id), `FilterString`)
}
topResultsTable$FILTER_ID<- unlist(sapply(topResultsTable$FILTER_ID,CSPName))
colnames(topResultsTable) <- c("FILTER","DrugName","Score")
topResults <-arrange(topResultsTable, Score)
```
