...
GenieSubsetsTable <- readRDS("genie.subsets.cn.profiles.rds")
...
makeDrugCNProfileTable <- function(cnlevels, drugIDs, threshold){
  ...
  for(drugID in drugIDs)
  {
    ...
    {
    drug.experiments <- filter(dose.response, DRUG_ID == drugID, LN_IC50 < threshold)
    records <- filter(gdsc.filtered.cn.bettertrans.df, COSMIC_ID %in% drug.experiments$COSMIC_ID)
...
```
Let's try plotting above in a faceted graph
```{r}
...
```{r}
getTumourDrugDist <- function(sampleid, sampleTable, drugid, drugTable, cnlevels){
  totalDist = 0
  for (level in cnlevels) {
    sampleprofile <- filter(sampleTable, SAMPLE_ID == sampleid, CN_LEVEL == level)[,c(-1, -2)]
    drugprofile <- filter(drugTable, DRUG_ID == drugid, CN_LEVEL == level)[,c(-1, -2)]
    totalDist <- totalDist + profileDist(sampleprofile, drugprofile)
  }
}

generateTumourDrugMatrix <- function(tumorprofile.df, drugprofile.df, cnlevels) {
  resultsTable <- data.frame()
  for (drugprofile in drugprofile.df){
    
    for (tumorprofile in tumorprofile.df){
...
#copy levels vector
cnlevels <- c(-2,-1,0,1,2)

```

```{r}

filterStrings <- data.frame(ID= c(1:66),
                 FilterString = c(""))
...
for (cancer in cancer_types){
  ...
  cancerPairedWithSubtypes[[name]] <- subtypes[subtypes >25]
