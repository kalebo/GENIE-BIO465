...
```{r}
#Drugs with most cell line response
strongestDrugs <- dose.response %>% group_by(DRUG_ID) %>% summarise(drugPower = sum(LN_IC50 < -2)) %>% arrange(desc(drugPower))
# Create GDSC Drug Id CNLevel Profile Table cnLevels v(), drugIds v()
makeDrugCNProfileTable <- function(cnlevels, drugIDs, threshold){
  headerLine <- c("DRUG_ID", "CN_LEVEL", unlist(intersecting.genes))
 gdsc.drug.cn.profile <- setNames(data.frame(matrix(ncol = length(headerLine), nrow = 0)), headerLine)
for(drugID in drugIDs)
{
  ...
  gdsc.drug.cn.profile <- rbind(drugID, copyLevel, unlist(profile))
  }
}
